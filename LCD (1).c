...
/* Pseudocode
		Enable clocks for GPIO A, B, C, and D
		Set GPIOA pins 6, 7, 8, 9, 10, and 15 to the proper AF
		Set GPIOB pins 0, 1, 4, 5, 9, 12, 13, 14, and 15 to the proper AF
		Set GPIOC pins 3, 4, 5, 6, 7, and 8 to the proper AF
		Set GPIOD pins 8, 9, 10, 11, 12, 13, 14, and 15 to the proper AF
*/
...
/* Pseudocode
		Wait until nobody else is pushing to the LED
		Set LCD ram bits to display 'Wilson'
		Request new setup to be displayed
*/
void LCD_DisplayName(void){
	//Wait until nobody else is pushing to the LED
	while((LCD->SR &= 0x00000004) != 0);
	
	//Set LCD ram bits to display 'Wilson'
	LCD->RAM[0] |= 0x02428210;
	LCD->RAM[1] |= 0x0000000E;
	LCD->RAM[2] |= 0x8680D148;
	LCD->RAM[3] |= 0x0000000B;
	LCD->RAM[4] |= 0x00000048;
	LCD->RAM[5] |= 0x00000000;
	LCD->RAM[6] |= 0x44021010;
...
/* Pseudocode
		Set bias to 1/3
		Set duty to 1/4
		Set max contrast value of 111
		Set pulse period to 111
		Disable MUX_SEG and set VSEL (voltage selection) to 0 for internal supply
		Wait for FCRSF flag to be set
		Enable the LCD
		Wait for the ENS bit to get set
		Wait for the ready bit to get set
*/
...
/* Pseudocode
		Wait until nobody else is pushing to the LED
		Set each SEG value in each LED ram array to 0
		Request new setup to be displayed
*/
void LCD_Clear(void){
	//Wait until nobody else is pushing to the LED
	while((LCD->SR &= 0x00000004) == 1);
...
/* Pseudocode
		Wait until nobody else is pushing to the LED
		Call LCD_WriteChar for the first six characters in ptr
*/
